...
mydwd <- function(D1, D2, c){
  n1 <- ncol(D1)
  n2 <- ncol(D2)
  f <- rep(1, rep(n1 + n2))
  f[1:n1] <- 0
  
  fit <- kdwd(x=t(cbind(D1,D2)), y=factor(f),scaled=TRUE,C=c)
  w <- fit@w[[1]]
  w <- w / norm(w)
  ...
  x.adj <- D1 - as.vector(mproj1 * w)
  y.adj <- D2 - as.vector(mproj2 * w)
  return (list(x.adj, y.adj))
}
...
eb = function(platform1.data, platform2.data, par.prior=TRUE, filter=FALSE, prior.plots=FALSE, p1.names=0, p2.names=0, skip.match=FALSE){
  ...
  #Call ComBat
  cb = ComBat(dat, saminfo, par.prior=par.prior, write=FALSE, filter=filter, prior.plots=prior.plots)
...
ComBat <- function(dat, saminfo, type='txt', write=F, covariates='all', par.prior=T, filter=F, skip=0, prior.plots=T){
