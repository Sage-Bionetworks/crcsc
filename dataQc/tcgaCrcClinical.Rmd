...
```
### Age Distribution
```{r age, include=FALSE, fig.height=4, fig.width=6}
ggplot(data=clinOut) + geom_density(aes(x=age), fill="blue", alpha=0.5)
```
### Gender
```{r gender, include=FALSE, results='asis'}
synTable(table(clinOut$gender))
```
### Tumor Stage
```{r stage, include=FALSE, results='asis'}
...
```
### Tumor Location
```{r location, include=FALSE, results='asis'}
...
### Mutation Status
#### KRAS
```{r kras, include=FALSE, results='asis'}
synTable(table(clinOut$krasStatus))
```
#### BRAF
```{r braf, include=FALSE, results='asis'}
synTable(table(clinOut$brafStatus))
```
#### NRAS
```{r nras, include=FALSE, results='asis'}
synTable(table(clinOut$nrasStatus))
```
#### EGFR
```{r egfr, include=FALSE, results='asis'}
synTable(table(clinOut$egfrStatus))
```
#### ALK
```{r alk, include=FALSE, results='asis'}
synTable(table(clinOut$alkStatus))
```
#### ERBB2
```{r erbb2, include=FALSE, results='asis'}
