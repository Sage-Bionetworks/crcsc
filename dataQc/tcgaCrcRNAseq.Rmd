...
act <- Activity(name="QC knitr script", used=list(readRNAseqSyn, readClinSyn, coadRNAseqSyn, coadClinSyn, list(url=rmdUrl, name=basename(rmdUrl), wasExecuted=TRUE)))
act <- synStore(act)
...
```
## QC output
This resulting tab delimited text files contain expression values on `r nrow(expr)` genes as well as clinical information on `r ncol(expr)-1` samples.
