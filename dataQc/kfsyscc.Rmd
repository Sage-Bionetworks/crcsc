...
rmdUrl <- getPermlink(crcscRepo, "dataQc/kfsyscc.Rmd")
...
clinFile <- synGet("syn2025168")
kfClin <- read.delim(getFileLocation(clinFile), as.is=T)

stopifnot(all(sapply(strsplit(sampleNames(kfExpr), ".", fixed=T), "[[", 1) == kfClin$SN))
rownames(kfClin) <- sampleNames(kfExpr)
kfClin$scanDate <- protocolData(kfRaw)$ScanDate
kfClin$scanDate <- chron(gsub(" .*", "", kfClin$scanDate))
pData(kfExpr) <- kfClin
kfRaw <- kfExpr


## ORDER SAMPLES IN CHRONOLOGICAL ORDER OF SCAN DATE
kfRaw <- kfRaw[, order(pData(kfRaw)$scanDate)]

## PERFORM SOME PRELIMINARY QC
pset <- fitPLM(kfRaw)
```
## Overview of the KFSYSCC data
KFSYSCC generously provided `r length(theseFiles)` CRC samples with expression profiling on the Affymetrix U133 Plus 2 platform.
...
clinOut <- data.frame(id=rownames(clin),
                      ...
                      dfsStat=NA,
                      osMo=clin$FT,
                      osStat=ifelse(clin$Death == 0 | is.na(clin$Death), 0, 1),
                      batch=clin$scanDate,
                      microsatelite=NA,
                      cimp=NA,
                      adjChemo=NA)
...
act <- Activity(name="QC knitr script", used=list(kfEnt, clinFile, list(url=rmdUrl, name=basename(rmdUrl), wasExecuted=TRUE)))
act <- synStore(act)
...
```
## QC output
The resulting tab delimited text files contain expression values on `r nrow(expr)` genes as well as clinical information on `r ncol(expr)-1`.
